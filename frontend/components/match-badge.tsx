...
export function MatchBadge({
  ...
  const cls = {
    ...
    none: "bg-fill-strong text-muted ring-white/8",
  }[tone];
  const dim = size === "sm" ? "h-6 min-w-6 px-1.5 text-[11px]" : "h-8 min-w-8 px-2 text-[13px]";
  return (
    <span className={`inline-flex items-center justify-center rounded-full font-mono font-medium tabular-nums ring-1 ${dim} ${cls}`}>
      {label}
    </span>
  );
}
